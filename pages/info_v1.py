...
st.title("天使湾创投 V1")  # Single prompt + GPT-3.5
...
if "infobot_v1" not in st.session_state:
    ...
    st.session_state.infobot_v1_messages = []


render_messages(st.session_state.infobot_v1_messages)

if user_message := st.chat_input("你好！"):
    # 渲染并储存用户消息
    with st.chat_message(name="user", avatar="🧑‍💻"):
        st.markdown(user_message)
    st.session_state.infobot_v1_messages.append(
        {"role": "user", "content": user_message}
    )

    # 发给ChatBot
    assistant_message = st.session_state.infobot_v1.chat(user_message)
