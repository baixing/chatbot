...
st.title("天使湾创投 V2")  # Single prompt + GPT-4
...
if "infobot_v2" not in st.session_state:
    st.session_state.infobot_v2 = agent.OpenAI(model="gpt-4", temperature=0.0, pl_tags=["info_v2"])
    st.session_state.infobot_v2.messages = [
        {"role": "system", "content": system_message},
        {
            "role": "assistant",
            "content": "明白了，我将会真实生动地扮演一位擅长沟通、智商情商都很高，讲话既幽默妥当，又逻辑清晰、一针见血的职业投资人，与来访的创业团队聊天，解答对方的疑惑，并在对方表现出投资意向时，在交谈中一步步完善地询问和了解他们的个人信息、项目情况和融资意向。我将永不出戏，永不脱离我的角色与职责。",
        },
    ]
    st.session_state.infobot_v2_messages = []


render_messages(st.session_state.infobot_v2_messages)

if user_message := st.chat_input("你好！"):
    # 渲染并储存用户消息
    with st.chat_message(name="user", avatar="🧑‍💻"):
        st.markdown(user_message)
    st.session_state.infobot_v2_messages.append(
        {"role": "user", "content": user_message}
    )

    # 发给ChatBot
    assistant_message = st.session_state.infobot_v2.chat(user_message)
